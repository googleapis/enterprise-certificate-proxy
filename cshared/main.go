...
func getCertPem(configFilePath string) []byte {
	...
	if err != nil {
		logger.Infof("Could not create client using config %s: %v", configFilePath, err)
		return nil
	}
	defer func() {
		if err = key.Close(); err != nil {
			logger.Errorf("Failed to clean up key. %v", err)
		}
	}()
...
func Sign(configFilePath *C.char, digest *byte, digestLen int, sigHolder *byte, sigHolderLen int) int {
	...
	if err != nil {
		logger.Infof("Could not create client using config %s: %v", C.GoString(configFilePath), err)
		return 0
	}
	defer func() {
		if err = key.Close(); err != nil {
	...
	case *ecdsa.PublicKey:
		isRsa = false
		logger.Info("the key is ecdsa key")
	case *rsa.PublicKey:
		isRsa = true
		logger.Info("the key is rsa key")
	default:
		logger.Infof("unsupported key type")
		return 0
	}
	...
	if signErr != nil {
		logger.Errorf("failed to sign hash: %v", signErr)
		return 0
	}
	if sigHolderLen < len(signature) {
		logger.Infof("The sigHolder buffer size %d is smaller than the signature size %d", sigHolderLen, len(signature))
		return 0
	}
